...
// FIX: The `sendMessage` function is not exported directly. Use `contentService` instead.
...
import { SendIcon, LoadingIcon } from './icons/index';
import StudioGuide from './StudioGuide';

const Chatbot: React.FC = () => {
  const [messages, setMessages] = useState<ChatMessage[]>([
    { sender: 'bot', text: "Hello! I'm your AI English assistant. Ask me anything about technical vocabulary, simulate an interview, or practice a code review." }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // FIX: The `sendMessage` function is not exported directly. Use `contentService` instead.
      // FIX: Argument of type '"gemini-2.5-flash"' is not assignable to parameter of type '"FAST" | "PRO"'. Changed to 'FAST'.
      const botResponse = await contentService.sendMessage(input, systemInstruction, 'FAST');
    ...
    <div className="flex flex-col h-full">
         <StudioGuide title="Guía de Estudio: AI Chatbot">
            <div>
                ...
                <p>Practicar conversaciones espontáneas y obtener explicaciones rápidas de términos técnicos.</p>
            </div>
            <div>
                ...
                <ul className="list-disc list-inside">
                    ...
                    <li>Simula escenarios (ej: "Let's role-play a stand-up meeting.").</li>
                </ul>
            </div>
             <div>
                <h3 className="font-semibold text-white">Pro Tip:</h3>
                <p>Usa el chatbot para prácticas diarias y rápidas. Esto te ayudará a construir confianza sin presión.</p>
            </div>
        </StudioGuide>
        <div className="flex flex-col flex-1 h-full bg-primary-medium rounded-xl border border-primary-light shadow-2xl">
            <div className="p-4 border-b border-primary-light">
                ...
                <p className="text-sm text-gray-400">Practice real-world tech conversations</p>
            </div>
            <div className="flex-1 p-6 overflow-y-auto space-y-6">
                ...
                <div key={index} className={`flex items-end gap-3 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                    ...
                    <div className={`max-w-md lg:max-w-xl px-4 py-3 rounded-2xl ${msg.sender === 'user' ? 'bg-accent-yellow text-dark-text rounded-br-none' : 'bg-primary-light text-light-text rounded-bl-none'}`}>
